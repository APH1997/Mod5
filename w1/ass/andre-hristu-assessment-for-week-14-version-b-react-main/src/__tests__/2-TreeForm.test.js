...
describe('(15 points) TreeForm', () => {
  ...
  test ('(1 points) TreeForm shows an error in "p.errors" if name is not filled in', () => {
    const treeForm = mount(
      <TreeForm />
    );
  
    expect(treeForm.find('input[name="name"]').props().value).toEqual("");
    expect(treeForm.find('p.errors').children()).toHaveLength(1);
  });
  
  test ('(1 points) TreeForm shows an error in "p.errors" if name more than 50 characters', () => {
    const treeForm = mount(
      <TreeForm />
    );
  ...
  test ('(1 points) TreeForm does not show any errors in "p.errors" if name filled in', () => {
    const treeForm = mount(
      <TreeForm />
    );
    ...
    expect(treeForm.find('input[name="name"]').props().value).toEqual("");
    treeForm.find('input[name="name"]').simulate('change', { target: { value: "Pear" } });
    expect(treeForm.find('p.errors').children()).toHaveLength(0);
  });
  
  test ('(1 points) TreeForm shows an error in "p.errors" if avgHeight is less than 1', () => {
    const treeForm = mount(
      <TreeForm />
    );
    ...
    expect(treeForm.find('p.errors').children()).toHaveLength(2);
  });
  
  test ('(1 points) TreeForm shows an error in "p.errors" if avgHeight is greater than 500', () => {
    const treeForm = mount(
      <TreeForm />
    );
